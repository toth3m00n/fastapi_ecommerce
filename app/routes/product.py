...
async def product_by_category(
    ...
    chosen_category_id = await db.scalar(
        select(Category.id).where(Category.slug == category_slug)
    )
    if chosen_category_id is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="There are no such category"
        )

    subcategories = await db.scalars(
        select(Category.id).where(Category.parent_id == chosen_category_id)
    )
...
async def product_detail(
    ...
    current_product = await db.scalar(
        select(Product).where(Product.slug == product_slug, Product.stock >= 0)
    )
